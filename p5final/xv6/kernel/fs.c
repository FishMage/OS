...
itrunc(struct inode *ip)
{
  ...
  uint *a;
  //regular file
  if(ip->type== T_SMALLFILE){//smallfile
    i = 0;
...
//Reading a SMALLFILE
  if(ip->type == T_SMALLFILE) 
  {
    ...
    if( n <= (NDIRECT+1)*4){
      char* data = (char*)ip->addrs + off;
      memmove(dst,data,n);
    }
    else if(n > (NDIRECT+1)*4)
    {
      return -1;
    }
   // ip->size = off;
  //  iupdate(ip); //if the inode changes then we need to update it
    return n;
  }
