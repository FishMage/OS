...
int main(int argc, char* argv[]){
	...
	char* searched_cmd= malloc(cmd_size);
		...
		else if(strcmp("exit",args[0])==0&&(legal == 1)){
			fclose(out_stream);
			exit(0);
		}
		else if((strcmp("cd",args[0])==0)&&(legal == 1)){
			if(num_args > 1){
				if(cd(args[1])<0){
					prompt_error();
					continue;
		...
		else if((strcmp("pwd",args[0])==0)&&(legal == 1)){
			pwd();
			continue;
		}
		else if((strcmp("path",args[0])==0)&&(legal == 1)){
			path_ctr = get_path();	
			continue;
		}else if((strcmp("\n",args[0])==0)&&(legal == 1)){
			continue;	
